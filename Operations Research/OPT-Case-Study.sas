...
/* declare variables */
	...
	impvar tanks_level{h in 0..24, nt in NETWORK_TANKS}  = 
		if (h=0) then(TANKS_INIT[nt])
			...
			else (if nt = "Ellerslie" then (Cornwall_flaw[h] - valves_schedule[h, "EricssonValve"] - NETWORK_DEMAND[h, "Ellerslie"])
				  else (if (nt = "Penrose") then (valves_schedule[h, "GreenwoodValve"] - Kingsland_flaw[h] - NETWORK_DEMAND[h, "Penrose"])
						else (if (nt = "Newmarket") then (valves_schedule[h, "EricssonValve"] + Kingsland_flaw[h] - NETWORK_DEMAND[h, "Newmarket"])
							 )
				  		)
			     )
			);
