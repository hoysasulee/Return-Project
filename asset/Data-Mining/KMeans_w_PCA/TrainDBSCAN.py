...
scaler = Normalizer().fit(df_train)
X = scaler.transform(df_train)
X = pd.DataFrame(X, index=df_train.index, columns = df_train.columns)
...
# model = DBSCAN(eps = 0.03, min_samples = 50, metric ='euclidean')
# y = model.fit_predict(X_pca)

model = GaussianMixture(n_components=3, reg_covar=1e-06)
y=model.fit_predict(X_pca)
print(y)
...
plt.scatter(X_pca[y == 3, 0], X_pca[y == 3, 1], s = 50, c = 'red', label = 'Cluster 4')
plt.scatter(X_pca[y == -1, 0], X_pca[y == -1, 1], s = 50, c = 'black', label = 'Noise')
