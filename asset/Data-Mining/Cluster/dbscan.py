...
def plotRes(data, clusterRes, clusterNum):
    ...
    scatterLabels = ['Noise', 'Cluster 1', 'Cluster 2', 'Cluster 3', 'Cluster 4', 'Cluster 5', 'Cluster 6']
    for i in range(clusterNum):
        color = scatterColors[i % len(scatterColors)]
        label = scatterLabels[i % len(scatterLabels)]
        x1 = [];  y1 = []
        for j in range(nPoints):
            if clusterRes[j] == i:
                x1.append(data[j, 0])
                y1.append(data[j, 1])
        plt.scatter(x1, y1, c=color, alpha=1, s=50, label= label)
        plt.title('Cluster of facial keypoints')
...
if __name__ == '__main__':
    data = load_data()
    cluster = np.asarray(data[:, 0])
    clusterRes, clusterNum = dbscan(data, 0.8, 5)
    plotRes(data, clusterRes, clusterNum)
    nmi, acc, purity = eva.eva(clusterRes, cluster)
    print("num", "acc", "purity")
